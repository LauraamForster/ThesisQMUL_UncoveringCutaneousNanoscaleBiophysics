...
DATA_ROOT = Path("/Volumes/LauraDrive/Multitech_Export_jeb")
...
SUBTYPE_LABELS = {
    ...
    "wt_rad": "WT RAD",
    "ko_rad": "KO RAD",
    "wt_vhneck": "WT VH neck",
    "ko_vhneck": "KO VH neck",
    "wt_radneck": "WT RAD neck",
    "ko_radneck": "KO RAD neck",
}
...
NI_COLOURS = {
    ...
    "wt_rad": "sandybrown",
    "ko_rad": "chocolate",
    "wt_vhneck": "lightsteelblue",
    "ko_vhneck": "steelblue",
    "wt_radneck": "khaki",
    "ko_radneck": "goldenrod",
}
...
SUBTYPE_ALIASES = {
    ...
    "wt_vhneck": "wt_vhneck",
    "wt vhneck": "wt_vhneck",
    "wt vh neck": "wt_vhneck",
    "wt vehicle neck": "wt_vhneck",

    "ko_vhneck": "ko_vhneck",
    "ko vhneck": "ko_vhneck",
    "ko vh neck": "ko_vhneck",
    "ko vehicle neck": "ko_vhneck",

    "wt_radneck": "wt_radneck",
    "wt radneck": "wt_radneck",
    "wt rad neck": "wt_radneck",
    "wt radiation neck": "wt_radneck",

    "ko_radneck": "ko_radneck",
    "ko radneck": "ko_radneck",
    "ko rad neck": "ko_radneck",
    "ko radiation neck": "ko_radneck",
...
def normalise_subtype(value):
    ...
    mapping = {
        ...
        "wt radneck": "wt_radneck",
        "wtradneck": "wt_radneck",
        "wt rad neck": "wt_radneck",

        "ko radneck": "ko_radneck",
        "koradneck": "ko_radneck",
        "ko rad neck": "ko_radneck",
    }
...
def plot_ni_raw_linescan_by_subtype_smooth(
    ...
    print_ni_linescan_position_values(
        smooth,
        positions=(0, 50, 100),
    )
...
def prepare_ni_raw_line_data(
    ...
    if layer_col is not None:
        ...
        print(
            f"[NI line prep] Dermis filter using {layer_col}: "
            f"{before} -> {len(d)} points"
        )
    
    else:
        print(
            "[NI line prep] WARNING: No dermis column found. "
            "Using all line-scan points."
        )
...
def print_ni_linescan_position_values(
    smooth,
    positions=(0, 50, 100),
):
    """
    Print smoothed NI values at selected normalised dermis positions.
    """

    if smooth.empty:
        print("No smoothed data available.")
        return

    print("\n" + "=" * 70)
    print("NI line scan values through dermis")
    print("=" * 70)

    for subtype in smooth["SubtypeClean"].astype(str).unique():

        d = smooth[
            smooth["SubtypeClean"].astype(str) == subtype
        ].copy()
...
def plot_ni_pca_scores(
    ...
    if print_scores:
        ...
        print("\n" + "=" * 80)
        print("NI PCA SUMMARY")
        print("=" * 80)

        print("\nExplained variance:")
        for i, val in enumerate(evr[:3], start=1):
            print(f"PC{i}: {val * 100:.2f}%")

        print("\nVariables:")
        print(list(pca_info["pca_vars"]))
        ...
        if "pca_components" in pca_info:
            ...
            loadings = pd.DataFrame(
                components[:2].T,
                index=pca_info["pca_vars"],
                columns=["PC1", "PC2"],
            )

        elif "loadings" in pca_info:
        ...
        if loadings is not None:
            ...
            print("\nNo PCA loading information found.")
            print("Available keys:")
            print(pca_info.keys())

        # ------------------------------------------------------------
        # Optional point scores
        # ------------------------------------------------------------
        if print_point_scores:
    ...
    for subtype in subtype_order:
        ...
        d_sub = pca_df[
            pca_df["SubtypeClean"].astype(str) == subtype
        ]
    ...
    ax.set_title(
        title or f"NI PCA: {', '.join(pca_info['pca_vars'])}"
    )
...
def print_saxs_bar_values(summary, title="SAXS bar values"):
    """
    Print values used for SAXS grouped bar plots.
    """
...
def plot_saxs_total_collagen_dermis_vs_wound(
    ...
    print_saxs_bar_values(
        summary,
        title=f"SAXS collagen intensity ({parameter}): dermis vs wound",
    )
...
def plot_saxs_total_collagen_4region(
    ...
    print_saxs_bar_values(
        summary,
        title=f"SAXS collagen intensity ({parameter}): 4 regions",
    )
    ...
    if run_stats:
        ...
        if not paired_stats.empty:
            ...
            if print_stats:
                ...
                print("\n[SAXS automatic paired spatial tests: lower/sub vs upper/epi regions]")
                print(
        ...
        if not between_stats.empty:
            ...
            if print_stats:
                ...
                print("\n[SAXS automatic between-subtype tests within each region]")
                print(
...
def plot_saxs_dperiod_dermis_vs_wound(
    ...
    # ------------------------------------------------------------
    # Prepare sample-level region summary
    # ------------------------------------------------------------
    ...
    print_saxs_bar_values(
        summary,
        title="SAXS D-period: dermis vs wound",
    )
...
def plot_saxs_dperiod_fwhm_dermis_vs_wound(
    ...
    Uses point-level SAXS D-period values to calculate a sample-level
    FWHM value for each sample/region.
    ...
    print_saxs_bar_values(
        summary,
        title="SAXS D-period FWHM: dermis vs wound",
    )
    ...
    if run_stats and fig is not None and ax is not None:
        ...
        if "Region" not in spread_samples.columns:
            ...
            if {"dermis", "wound"}.issubset(regions_present):
                ...
                # Rename FWHM to SampleMean so the general stats function can use it
                stats_input = spread_samples.rename(
                    columns={
                        "FWHM": "SampleMean"
                    }
                ).copy()
...
def plot_saxs_peak_position_bar(
    ...
    print_saxs_bar_values(
        summary,
        title="SAXS folded peak position by subtype and region",
    )
    ...
    if run_stats and fig is not None and ax is not None:
        ...
        if "Region" not in sample_means.columns:
            ...
            # Case 1: paired dermis vs wound within each subtype
            # ------------------------------------------------------------
            if {"dermis", "wound"}.issubset(regions_present) and {"dermis", "wound"}.issubset(requested_regions):
            ...
            # ------------------------------------------------------------
            # Case 2: one plotted region, or non-paired region structure:
            # compare between subtypes within each region
            # ------------------------------------------------------------
            else:
...
def plot_saxs_peak_spread_dermis_vs_wound(
    ...
    # ------------------------------------------------------------
    # Calculate sample-level peak spread
    # ------------------------------------------------------------
    ...
    print_saxs_bar_values(
        spread_summary,
        title="SAXS peak spread dermis vs wound",
    )
    ...
    if run_stats and spread_samples is not None and not spread_samples.empty:
        ...
        # ------------------------------------------------------------
        # Work out the value column used for spread
        # ------------------------------------------------------------
        possible_value_cols = [
            "SpreadDegrees",
            "Spread",
...
def plot_saxs_peak_spread_4region(
    ...
    if spread_summary is None or spread_summary.empty:
        print("No SAXS peak-spread split-region data available.")
        return None, None, pd.DataFrame()

    print_saxs_bar_values(
        spread_summary,
        title="SAXS peak spread 4 regions",
    )
    ...
    if run_stats and spread_samples is not None and not spread_samples.empty:
        ...
        if value_col is None:
            ...
            # General stats helpers expect the value column to be SampleMean
            stats_input = stats_input.rename(
                columns={
                    value_col: "SampleMean"
                }
...
def plot_saxs_peak_spread_shift_lower_to_upper(
    ...
    """
    % shift in peak-position spread:
        lower dermis -> upper dermis
        lower wound  -> upper wound

    The shift values are calculated per sample.

    Statistical testing is performed on sample-level PercentShift values.

    Because the shift already represents an upper-vs-lower comparison
    within each sample, the automatic statistics compare PercentShift
    values between subtypes for each ShiftType.
    """
    ...
    if shift_summary is None or shift_summary.empty:
        print("No SAXS peak-spread lower-to-upper shift data available.")
        return None, None, pd.DataFrame()

    print_saxs_bar_values(
        shift_summary,
        title="SAXS peak spread lower-to-upper shift",
    )
    ...
    fig, ax = _plot_saxs_peak_spread_shift_bars(
        shift_df=shift_df,
        shift_summary=shift_summary,
    ...
    if run_stats and shift_df is not None and not shift_df.empty:
        ...
        if "ShiftType" not in shift_df.columns or "PercentShift" not in shift_df.columns:

            print("\n[SAXS peak-spread shift stats] Expected long-format shift columns not found.")
            print("Expected columns: ShiftType, PercentShift")
            print("Available columns:")
            ...
            if print_stats and not stats_df.empty:
                ...
                print("\n[SAXS automatic between-subtype tests: peak-spread lower-to-upper shift]")
                print(
...
has_raw_raman = (
    raw_raman_df is not None
    and not raw_raman_df.empty
    and all(col in raw_raman_df.columns for col in RAMAN_REQUIRED_RAW_COLS)
)
...
if has_raw_raman:
    ...
    print("No usable RawRaman data found; skipping Raman spectra setup.")
    raman_spectra = pd.DataFrame()


RAMAN_REQUIRED_WM_COLS = [
    "Sample",
    "Subtype",
]

has_raman_summary = (
    raman_df is not None
    and not raman_df.empty
    and all(col in raman_df.columns for col in RAMAN_REQUIRED_WM_COLS)
)

if has_raman_summary:
    raman_wm_long = tidy_raman_weighted_moments(raman_df)
else:
    print("No usable Raman summary data found; skipping Raman weighted moments setup.")
    raman_wm_long = pd.DataFrame()
...
fig, ax = plot_ni_pca_by_position_with_alpha(
    pca_df=ni_pca_df,
    pca_info=ni_pca_info,
    subtype_order=subtypes_to_plot,
    colours=NI_COLOURS,
    markers=NI_MARKERS,
    alpha_min=0.20,
    alpha_max=0.90,
...
fig, ax, saxs_peak_spread_shift_stats = plot_saxs_peak_spread_shift_lower_to_upper(
    saxs_points=tables["saxs_points"],
    subtypes_to_plot=subtypes_to_plot,
    hatches=False,
    run_stats=True,
