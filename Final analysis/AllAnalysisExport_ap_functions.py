...
def NI_build_ni_csv_path(base_path, set_type, sample_type, folder, canon_reg, original_sample_type=None):
    ...
    Current AP1 folder structure:

        base_path/
            TS/
                OutputExcel_TS1_linescan.csv
            VH/
                OutputExcel_VH1_linescan.csv
            AC/
                OutputExcel_AC1_linescan.csv
            CL/
                OutputExcel_CL1_linescan.csv

    Parameters
    ----------
    base_path : str or Path
        Base nanoindentation CSV folder, e.g.
        /Volumes/LauraDrive/Nanoindentation/NI_Data/Output/CSV/AP1

    set_type : str
        Experiment type. Currently supports AP1.

    sample_type : str
        Normalised subtype, e.g. ts, vh, ac, cl.

    folder : str
        Folder/sample name from manifest, e.g. TS1, VH2, AC1, CL3.

    canon_reg : str
        Region requested. For AP1 this should be line scan / linescan.

    original_sample_type : str, optional
        Original TYPE label from manifest, used for robust folder matching.
    """
    ...
    if st_lower == "ap1":
        ...
        if reg_key == "horiz_linescan":
            filename_options = [
                f"OutputExcel_{folder}_horiz_linescan.csv",
                f"OutputExcel_{folder}_horizlinescan.csv",
                f"OutputExcel_{folder}_horizontal_linescan.csv",
        ...
        # Return the most likely path so missing-file reporting is useful
