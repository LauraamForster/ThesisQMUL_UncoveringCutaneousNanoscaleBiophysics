...
# Input setup
# =============================================================================
DATA_ROOT = Path("/Volumes/LauraDrive/Multitech_Export_ap1")

SUBTYPES =[ "control", "ac", "cl", "ts", "vh"]

SUBTYPE_ORDER = ["control", "vh", "ts", "ac", "cl"]
...
def read_one_workbook(path, subtype, sheets=SHEETS):
    ...
    for sheet in sheets:
        if sheet not in available_set:
            # These are optional because old/new export formats differ.
            if sheet not in optional_format_sheets:
...
def split_loaded_tables(
    ...
    # Cell data
    # -------------------------
    if cell_df is not None and not cell_df.empty and "ExportLevel" in cell_df.columns:
    ...
    # SAXS
    # -------------------------
    if saxs_df is not None and not saxs_df.empty and "ExportLevel" in saxs_df.columns:
    ...
    if ni_df is not None and not ni_df.empty:
        ...
        if "ExportLevel" in ni_df.columns:
            tables["ni_summary"] = ni_df[ni_df["ExportLevel"] == "SampleSummary"].copy()
            tables["ni_binned"] = ni_df[ni_df["ExportLevel"] == "Binned"].copy()

        else:
            # Old-style fallback.
            # If there is a Bin column, infer binned vs summary.
            if "Bin" in ni_df.columns:
                tables["ni_binned"] = ni_df[ni_df["Bin"].notna()].copy()
                tables["ni_summary"] = ni_df[ni_df["Bin"].isna()].copy()
            else:
                tables["ni_summary"] = ni_df.copy()
...
def print_ni_smooth_values_at_positions(
            ...
            rows.append(
                {
                    ...
                    "RequestedPosition_%": pos,
                    "ActualPosition_%": s_sub.loc[idx, "NormalisedPosition"],
                    "Mean": s_sub.loc[idx, value_col],
    ...
    if out.empty:
        print("No smoothed position values found.")
        return out

    print("\n" + "=" * 80)
    print("NI smoothed values at selected positions")
    print("=" * 80)
    print(out.to_string(index=False))

    return out

def print_ni_binned_bar_summary(summary, labels=NI_LABELS, title="NI binned bar summary"):
    """
    Print mean, std, sem, n, and error for each subtype/bin bar.
    """
    ...
    s["SubtypeLabel"] = s["SubtypeClean"].astype(str).map(
...
def print_ni_pca_loadings(ni_pca_info, pca_vars=None):
    ...
    # Your prepare_ni_pca_data stores names/loadings under these keys
    ...
    # Fallback for sklearn-style PCA info if used elsewhere
    if loadings is None and "components" in ni_pca_info:
        loadings = np.asarray(ni_pca_info["components"], dtype=float).T

    if loadings is None and "pca" in ni_pca_info:
        loadings = np.asarray(ni_pca_info["pca"].components_, dtype=float).T

        if explained is None:
            explained = ni_pca_info["pca"].explained_variance_ratio_
    ...
    print("\n" + "=" * 80)
    print("NI PCA explained variance")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("NI PCA loadings")
    print("=" * 80)
    print(loadings_df.round(3).to_string())

    print("\n" + "=" * 80)
    print("Main contributors by absolute loading")
    print("=" * 80)
...
def plot_ni_raw_linescan_by_subtype_smooth(
    ...
    Also prints smoothed values at selected normalised positions.
    """
    ...
    if panel:
        ...
        return fig, axes, smooth, ni_position_values
    ...
    return fig, ax, smooth, ni_position_values
...
def plot_ni_binned_bar_by_position(
    ...
    if print_summary:
        print_ni_binned_bar_summary(
            summary,
            labels=labels,
            title=f"NI binned bar summary: {variable}"
        )
...
def print_bar_values_to_console(
    summary,
    title="Bar summary",
    labels=NI_LABELS,
):
    """
    Print bar mean/error values to console.

    Expects summary to contain some of:
        SubtypeClean, Region, shift, mean, std, sem, n, error, err
    """
    ...
    if "SubtypeClean" in s.columns:
        s["Label"] = s["SubtypeClean"].astype(str).map(lambda x: labels.get(x, x))

    # Standardise error column name if possible
    if "error" not in s.columns:
        if "err" in s.columns:
            s["error"] = s["err"]
        elif "std" in s.columns:
            s["error"] = s["std"]

    if "sem" not in s.columns and {"std", "n"}.issubset(s.columns):
        s["sem"] = s["std"] / np.sqrt(s["n"])
...
def plot_saxs_total_collagen_dermis_vs_wound(
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            summary,
            title="SAXS total collagen dermis vs wound bar values",
            labels=labels,
        )
    ...
    if paired_test and fig is not None and ax is not None:
        ...
        if "Region" not in sample_means.columns:
            ...
            print("\n[SAXS stats] No Region column found in sample_means.")
            ...
            if {"dermis", "wound"}.issubset(regions_present):
                ...
                if print_stats and not stats_df.empty:
                    ...
                    print("\n[SAXS automatic paired tests: dermis vs wound]")
                    print(
            ...
            # ------------------------------------------------------------
                ...
                if print_stats and not stats_df.empty:
                    ...
                    print("\n[SAXS automatic between-subtype tests]")
                    print(
...
def calculate_saxs_dermis_to_wound_percent_shift(
    ...
    """
    Calculate matched dermis -> wound % shift for SAXS parameter.
    ...
    if d.empty:
        print("No dermis/wound SAXS data available.")
        return pd.DataFrame(), pd.DataFrame()

    sample_means, _ = summarise_saxs_points_to_sample_bars(
        d,
        value_col=plot_value_col,
        region_col="RegionForPlot",
    )
    ...
    # experiment, subtype, SubtypeClean, Filenumber, Region, SampleMean
    wide = (
        sample_means
        .pivot_table(
    ...
    if "dermis" not in wide.columns or "wound" not in wide.columns:
        print("Could not calculate % shift because dermis or wound column is missing.")
        print("Available columns:", wide.columns.tolist())
        return pd.DataFrame(), pd.DataFrame()

    wide["dermis"] = pd.to_numeric(wide["dermis"], errors="coerce")
    wide["wound"] = pd.to_numeric(wide["wound"], errors="coerce")

    shift_df = wide.dropna(subset=["dermis", "wound"]).copy()
    shift_df = shift_df[shift_df["dermis"] != 0].copy()

    if shift_df.empty:
        print("No matched dermis/wound sample pairs available for % shift.")
        return pd.DataFrame(), pd.DataFrame()

    shift_df["PercentShift_Wound_vs_Dermis"] = (
        (shift_df["wound"] - shift_df["dermis"]) / shift_df["dermis"]
    ) * 100

    shift_df = shift_df[
        shift_df["SubtypeClean"].astype(str).isin(subtype_order)
    ].copy()

    if shift_df.empty:
        print("No matched % shift data left after subtype filtering.")
        return pd.DataFrame(), pd.DataFrame()

    summary = (
        shift_df
        .groupby("SubtypeClean", observed=True)["PercentShift_Wound_vs_Dermis"]
        .agg(mean="mean", std="std", n="count")
        .reset_index()
    )
    ...
    summary["SubtypeClean"] = pd.Categorical(
        summary["SubtypeClean"],
        categories=subtype_order,
        ordered=True,
    )

    summary = summary.sort_values("SubtypeClean").reset_index(drop=True)

    return shift_df, summary

def plot_saxs_dermis_to_wound_percent_shift(
    saxs_points,
    parameter="curvearea",
    normalise=True,
    subtype_order=SUBTYPE_ORDER,
    ...
    """
    Plot dermis -> wound % shift for a SAXS parameter.

    For each matched sample:
        % shift = ((wound - dermis) / dermis) * 100

    Positive:
        wound is higher than dermis

    Negative:
        wound is lower than dermis
    """
    ...
    if summary.empty:
        ...
        return None, None, shift_df, summary
    ...
    if not present_subtypes:
        print("No matching subtypes in % shift summary.")
        return None, None, shift_df, summary
    ...
    if title is None:
        title = "SAXS dermis-to-wound % shift"

    if print_summary:
        print_bar_values_to_console(
            summary,
            title="SAXS dermis-to-wound % shift bar values",
            labels=labels,
        )

        print("\nMatched sample-level % shifts:")
        print(
            shift_df[
                [
                    "SubtypeClean",
                    ...
                    "dermis",
                    "wound",
                    "PercentShift_Wound_vs_Dermis",
                ]
            ].to_string(index=False)
        )

    fig, ax = plt.subplots(figsize=(max(6, len(present_subtypes) * 1.1), 5))
    ...
    means = s_plot["mean"].to_numpy(dtype=float)
    errors = s_plot["error"].to_numpy(dtype=float)
    ...
    if scatter:
        ...
        for i, subtype in enumerate(present_subtypes):
            vals = shift_df[
                shift_df["SubtypeClean"].astype(str) == subtype
            ]["PercentShift_Wound_vs_Dermis"].dropna().to_numpy(dtype=float)
    ...
    return fig, ax, shift_df, summary
...
def plot_saxs_total_collagen_4region(
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            summary,
            title="SAXS total collagen 4-region bar values",
            labels=labels,
        )
    ...
    if run_stats and fig is not None and ax is not None:
        ...
        if "Region" not in sample_means.columns:
            ...
            print("\n[SAXS 4-region stats] No Region column found in sample_means.")
            print("Available columns:")
            print(sample_means.columns.tolist())
...
def plot_saxs_dperiod_dermis_vs_wound(
    ...
    # ------------------------------------------------------------
    # Prepare sample-level D-period summary
    # ------------------------------------------------------------
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            summary,
            title="SAXS D-period dermis vs wound bar values",
            labels=labels,
        )
...
def plot_saxs_dperiod_4region(
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            summary,
            title="SAXS D-period 4-region bar values",
            labels=labels,
        )
...
def plot_saxs_dperiod_shift_dermis_to_wound(
    ...
    The shift is calculated per matched sample as:
        wound_vs_dermis

    Statistical testing is performed on sample-level % shift values.

    Because the shift already represents a paired dermis-to-wound comparison
    within each sample, the automatic statistics compare shift values
    between subtypes.
    ...
    # ------------------------------------------------------------
    # Calculate sample-level dermis-to-wound D-period shifts
    # ------------------------------------------------------------
    ...
    if summary is None or summary.empty:
        print("No SAXS D-period dermis-to-wound shift data available.")
        return None, None, pd.DataFrame()

    # ------------------------------------------------------------
    # Keep only requested subtypes
    # ------------------------------------------------------------
    summary = summary[
        summary["SubtypeClean"].astype(str).isin(subtype_order)
    ].copy()
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            summary,
            title="SAXS D-period dermis-to-wound % shift bar values",
            labels=labels,
        )

        if shift_df is not None and not shift_df.empty:
            print("\nMatched sample-level D-period shifts:")
            print(
    ...
    if run_stats and shift_df is not None and not shift_df.empty:

        # ------------------------------------------------------------
        # Case 1: wide-format shift_df
        # Expected column:
        #     wound_vs_dermis
        # ------------------------------------------------------------
        if "wound_vs_dermis" in shift_df.columns:

            stats_df = auto_saxs_shift_between_subtype_tests(
                shift_df=shift_df,
                shift_value_cols=["wound_vs_dermis"],
                value_labels={
                    "wound_vs_dermis": "Wound vs dermis",
                },
        ...
        # ------------------------------------------------------------
        # Case 2: long-format shift_df
        # Expected columns:
        #     ShiftType
        #     PercentShift
        # ------------------------------------------------------------
        elif {"ShiftType", "PercentShift"}.issubset(shift_df.columns):

            stats_df = auto_saxs_long_shift_between_subtype_tests(
                shift_df=shift_df,
            ...
            print("\n[SAXS D-period shift stats] Could not find shift value column.")
            print("Expected either:")
            print("  wide format: column 'wound_vs_dermis'")
            print("  long format: columns 'ShiftType' and 'PercentShift'")
            print("Available columns:")
            print(shift_df.columns.tolist())

        if print_stats and stats_df is not None and not stats_df.empty:
            ...
            print("\n[SAXS automatic between-subtype tests: D-period dermis-to-wound shift]")
            print(
                stats_print[stats_cols]
                .to_string(index=False)
            )
...
def plot_saxs_dperiod_fwhm_dermis_vs_wound(
    ...
    Uses point-level SAXS D-period values to calculate sample-level
    FWHM values for each sample/region.
    ...
    # ------------------------------------------------------------
    # Prepare sample-level D-period FWHM/spread values
    # ------------------------------------------------------------
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            summary,
            title="SAXS D-period FWHM dermis vs wound bar values",
            labels=labels,
        )
...
def plot_saxs_dperiod_fwhm_4region(
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            summary,
            title="SAXS D-period FWHM 4-region bar values",
            labels=labels,
        )
...
def plot_saxs_peak_position_bar(
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            summary,
            title="SAXS folded peak-position bar values",
            labels=labels,
        )
    ...
    if run_stats and fig is not None and ax is not None:
        ...
        if "Region" not in sample_means.columns:
            ...
            print("\n[SAXS peak-position stats] No Region column found in sample_means.")
            print("Available columns:")
            print(sample_means.columns.tolist())
...
def plot_saxs_peak_position_shift_from_dermis(
    ...
    # ------------------------------------------------------------
    # Calculate sample-level folded peak-position shifts
    # ------------------------------------------------------------
    ...
    if summary is None or summary.empty:
        print("No SAXS folded peak-position shift data available.")
        return None, None, pd.DataFrame()
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            summary,
            title="SAXS folded peak-position % shift bar values",
            labels=labels,
        )

        if shift_df is not None and not shift_df.empty:
            print("\nMatched sample-level folded peak-position shifts:")
            print(
                shift_df.to_string(index=False)
            )
    ...
    if run_stats and shift_df is not None and not shift_df.empty:
        ...
        # ------------------------------------------------------------
        # Case 1: long-format shift_df already exists
        # Expected columns:
        #     ShiftType
        #     PercentShift
        # ------------------------------------------------------------
        if {"ShiftType", "PercentShift"}.issubset(shift_df.columns):

            stats_input = shift_df.copy()

            stats_df = auto_saxs_long_shift_between_subtype_tests(
                shift_df=stats_input,
                shift_type_col="ShiftType",
                value_col="PercentShift",
                subtype_col="SubtypeClean",
                sample_col=None,  # auto-detects Filenumber / SampleKey / etc.
        ...
        # ------------------------------------------------------------
        # Case 2: wide-format shift_df
        # Expected column:
        #     wound_vs_dermis
        # Convert to long format for the existing helper.
        # ------------------------------------------------------------
        elif "wound_vs_dermis" in shift_df.columns:
            ...
            stats_input = shift_df[
                id_cols + ["wound_vs_dermis"]
            ].copy()

            stats_input = stats_input.rename(
                columns={
                    "wound_vs_dermis": "PercentShift",
                }
            )

            stats_input["ShiftType"] = "wound_vs_dermis"
            ...
            print("\n[SAXS folded peak-position shift stats] Could not find shift value column.")
            print("Expected either:")
            print("  wide format: column 'wound_vs_dermis'")
            print("  long format: columns 'ShiftType' and 'PercentShift'")
            print("Available columns:")
        ...
        if print_stats and stats_df is not None and not stats_df.empty:
            ...
            print("\n[SAXS automatic between-subtype tests: folded peak-position dermis-to-wound shift]")
            print(
                stats_print[stats_cols]
                .to_string(index=False)
            )
...
def plot_saxs_peak_spread_dermis_vs_wound(
    ...
    if spread_summary is None or spread_summary.empty:
        print("No SAXS peak-position spread dermis/wound data available.")
        return None, None, pd.DataFrame()
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            spread_summary,
            title="SAXS peak-position spread dermis vs wound bar values",
            labels=labels,
        )
    ...
    if run_stats and spread_samples is not None and not spread_samples.empty:
        ...
        # ------------------------------------------------------------
        # Work out the spread value column
        # ------------------------------------------------------------
        possible_value_cols = [
            "SpreadDegrees",
            "Spread",
        ...
        if value_col is None:
            ...
            print("\n[SAXS peak-position spread stats] Could not find spread value column.")
            print("Available columns:")
            print(stats_input.columns.tolist())

        elif "Region" not in stats_input.columns:

            print("\n[SAXS peak-position spread stats] No Region column found in spread_samples.")
            print("Available columns:")
            print(stats_input.columns.tolist())
...
def plot_saxs_peak_spread_4region(
    ...
    if spread_summary is None or spread_summary.empty:
        print("No SAXS peak-position spread 4-region data available.")
        return None, None, pd.DataFrame()
    ...
    if print_bar_summary:
        print_bar_values_to_console(
            spread_summary,
            title="SAXS peak-position spread 4-region bar values",
            labels=labels,
        )
    ...
    if run_stats and spread_samples is not None and not spread_samples.empty:
        ...
        if value_col is None:
            ...
            print("\n[SAXS peak-position spread 4-region stats] Could not find spread value column.")
            print("Available columns:")
            print(stats_input.columns.tolist())

        elif "Region" not in stats_input.columns:

            print("\n[SAXS peak-position spread 4-region stats] No Region column found in spread_samples.")
            print("Available columns:")
            print(stats_input.columns.tolist())
...
# Old control/wounding NI export format
...
# Grid NI only exists in newer AP1/JEB/Bleo files
...
if raw_raman_df is not None and not raw_raman_df.empty:
    ...
    raman_spectra = pd.DataFrame()
    print("[AP1] No raw Raman data available.")

if raman_df is not None and not raman_df.empty:
    raman_wm_long = tidy_raman_weighted_moments(raman_df)
else:
    raman_wm_long = pd.DataFrame()
    print("[AP1] No Raman weighted-moment summary available.")
...
fig, ax, ni_modulus_smooth, ni_modulus_position_values = plot_ni_raw_linescan_by_subtype_smooth(
    ni_raw=ni_raw_screened,
    variable="mod_Hertz",
    ylabel="Indentation modulus (kPa)",
    title=" ",
    subtypes_to_plot=[ "ac", "cl"],
    window=18,
    grid_size=200,
...
fig, ax, ni_tau_smooth, ni_modulus_position_values = plot_ni_raw_linescan_by_subtype_smooth(
    ni_raw=ni_raw_screened,
    variable="tau_Visco",
    ylabel="Tau (s)",
    subtypes_to_plot=["ac", "cl"],
...
fig, ax, saxs_shift_df, saxs_shift_summary = plot_saxs_dermis_to_wound_percent_shift(
    saxs_points=tables["saxs_points"],
    parameter="curvearea",
    normalise=True,
    subtypes_to_plot=["ts", "vh", "ac", "cl"],
...
fig, ax, saxs_dperiod_shift_stats = plot_saxs_dperiod_shift_dermis_to_wound(
    saxs_points=tables["saxs_points"],
    subtypes_to_plot=["ts", "vh"],
    hatches=False,
    print_bar_summary=True,
    run_stats=True,
    print_stats=True,
...
fig, ax, saxs_peak_position_shift_stats = plot_saxs_peak_position_shift_from_dermis(
    saxs_points=tables["saxs_points"],
    min_rsq=0.3,
    subtypes_to_plot=["ts", "vh"],
    hatches=False,
...
fig_2x2, axes_2x2 = plot_raman_pca_scores_2x2_by_normpos(
    raman_pca_scores,
    subtype_order=["ts", "vh", "ac", "cl"],
    pcx="PC1",
    pcy="PC2",
    norm_col="NormPos",
    cmap="jet",
    title="Within-subtype Raman PCA: PC1 vs PC2 by spatial position",
)

fig_np, axes_np, raman_pca_scores_with_normpos, raman_pca_info_with_normpos = plot_raman_pca_with_normpos_as_feature_2x2(
    raman_spectra,
    subtype_order=["ts", "vh", "ac", "cl"],
    xlim=(1200, 1800),
    standardise=True,
    normpos_weight=1.0,
    cmap="jet",
    title="Within-subtype Raman PCA including normalised position as feature",
)
# -----
...
fig, ax, raman_selected_wm = plot_raman_selected_weighted_moments(
    raman_wm_long,
    peak_regions=["CH2CH3_RIGHT_1330_1380"],
