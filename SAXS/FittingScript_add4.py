...
def ProcessIQFitting(scan_no, Output_directoryCSV, base_output_path, Output_directorybsd, sample_loc,
    ...
    # Use local copies so call sites and other code remain unchanged
    local_min_area_threshold = 0.0 if TEST_MODE else float(minimum_area_threshold)
    local_rsq_min            = 0.0 if TEST_MODE else float(rsq_min)
    ...
    # Safety: all columns must match length
    n = len(ixt)
...
def process_order_peak(order_no, order_position, df_lookup, moment_mat, diode_val, x, y,
    ...
    # y_peak_filtered  = gaussian_filter1d(y_peak,  6)
    # y_sides_filtered = gaussian_filter1d(y_sides, 6)
...
def ProcessICHIFitting(
    ...
    def compute_stats(hv, sv):
        ...
        total_area = float(np.trapezoid(np.clip(hv, 0.0, None), chi_vals))
        peak_area = float(np.trapezoid(pos, chi_vals))
...
def calculate_alignment_param(
    ...
    try:
        ...
        area_fit = float(np.trapezoid(y_fit, chi))
