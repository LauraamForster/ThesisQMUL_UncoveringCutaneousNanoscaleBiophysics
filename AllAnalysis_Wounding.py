...
DATA_ROOT = Path("/Volumes/LauraDrive/Multitech_Export")

SUBTYPES = ["control", "d4", "d7", "d10", "d14", "d21"]
...
SUBTYPE_ORDER = ["control", "d4", "d7", "d10", "d14", "d21"]
...
RAMAN_PEAK_REGIONS = [
    ("AMIDEIII_LEFT_1220_1330", (1220, 1300)),
    ("AMIDEIII_RIGHT_1330_1380", (1310, 1380)),
...
def read_one_workbook(path, subtype, sheets=SHEETS):
    """
    Read one subtype workbook safely.
    Handles missing/corrupt workbooks without crashing the whole analysis.
    """
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print(f"Could not open workbook for subtype '{subtype}':")
        print(path)
        print(f"Error: {type(exc).__name__}: {exc}")
        print("This file may be corrupt, half-written, open/locked, or not a valid .xlsx.")
        print("=" * 80)
    ...
    for sheet in sheets:
        ...
        try:
            ...
            print(f"Could not read sheet '{sheet}' in {path.name}: {exc}")
            df = pd.DataFrame()
...
# STATS
# =============================================================================
# Generic sample-level stats helpers
# =============================================================================

def _auto_find_sample_col(df, sample_col=None):
    """
    Find sample column automatically if not supplied.
    """

    if sample_col is not None:
        if sample_col not in df.columns:
            raise KeyError(f"sample_col '{sample_col}' not found.")
        return sample_col
    ...
    raise KeyError(
        "Could not automatically find a sample column. "
        "Pass sample_col='your_sample_column_name'. "
        f"Available columns are: {list(df.columns)}"
    )

def run_anova_between_subtypes(
    df,
    *,
    value_col,
    ...
    """
    Run one-way ANOVA between subtypes using sample-level means.

    Designed for comparisons such as:
        Unwounded vs PW4 vs PW7 vs PW10 vs PW21

    Parameters
    ----------
    df : pd.DataFrame
        Input data. Can be raw point-level data or already sample-level data.

    value_col : str
        Column containing the value to test.

    subtype_col : str
        Column containing subtype/group labels.

    sample_col : str or None
        Sample ID column. If None, this is auto-detected.

    subtype_order : list or tuple or None
        Order of subtypes to include.

    group_cols : list or tuple or None
        Extra columns to run separate ANOVAs within.

        Examples:
            group_cols=["Region"]
            group_cols=["PeakRegion", "Metric"]
            group_cols=["Region", "Metric"]

        If None, one ANOVA is run across the whole dataframe.

    label_map : dict or None
        Optional mapping for display labels.

    Returns
    -------
    stats_df : pd.DataFrame
        One row per ANOVA comparison.
    """
    ...
    if value_col not in d.columns:
        raise KeyError(f"value_col '{value_col}' not found.")

    if subtype_col not in d.columns:
        raise KeyError(f"subtype_col '{subtype_col}' not found.")

    sample_col = _auto_find_sample_col(d, sample_col=sample_col)
    ...
    if missing_group_cols:
        raise KeyError(f"Missing group_cols: {missing_group_cols}")
    ...
    if subtype_order is None:
        subtype_order = list(pd.unique(d[subtype_col]))
    else:
        subtype_order = [str(st).strip() for st in subtype_order]

    d = d[d[subtype_col].isin(subtype_order)].copy()

    if d.empty:
        return pd.DataFrame()

    # ------------------------------------------------------------
    # First reduce to sample-level means
    # ------------------------------------------------------------
    sample_group_cols = group_cols + [
        subtype_col,
        sample_col,
    ]

    sample_means = (
        d.groupby(sample_group_cols, as_index=False, observed=True)[value_col]
        .mean()
    ...
    if group_cols:
        grouped = sample_means.groupby(group_cols, observed=True, dropna=False)
    else:
        sample_means["_AllData"] = "All"
        grouped = sample_means.groupby(["_AllData"], observed=True, dropna=False)
    ...
    for group_key, g in grouped:
        ...
        for subtype in subtype_order:
            vals = (
                g.loc[
                    g[subtype_col].astype(str) == str(subtype),
                    "SampleMean",
                ]
                .dropna()
        ...
        comparison = " vs ".join(
            label_map.get(st, st) if label_map else st
            for st in group_names
        )

        row.update(
            {
                "ComparisonType": "between-subtype one-way ANOVA",
                "Comparison": comparison,
                "SelectedTest": "one-way ANOVA",
                "ValueColumn": value_col,
                "SubtypeColumn": subtype_col,
                "SampleColumn": sample_col,
                "n_groups": len(groups),
                "GroupNs": ", ".join(
                    f"{label_map.get(name, name) if label_map else name}: n={len(vals)}"
    ...
    if print_stats:
        print("\n[One-way ANOVA between subtypes]")
        if stats_df.empty:
            print("No ANOVA tests were run.")
        else:
...
def screen_ni_raw_dataframe(
    ...
    This sets failed values to NaN so downstream plotting functions
    ...
    Screening:
        mod_Hertz:
            ...
            value >= lo_modulus_pa
            value <= hi_modulus_pa
            group-wise mean ± std_devs * SD trimming

        tau_Visco:
            Rsq_Visco / Rsq_ViscoAna / ViscoAna_r2 >= rsq_min_visco
            value >= 0
            group-wise mean ± std_devs * SD trimming
    ...
    # Use the most local grouping possible for SD trimming.
    group_cols = [
    ...
    for variable in variables:
        ...
        if variable == "mod_Hertz":
            if "Rsq_Hertz" in d.columns:
                ...
                print("[NI screening] Rsq_Hertz column not found; mod_Hertz Rsq screening skipped.")
            ...
            if rsq_col is not None:
                ...
                print("[NI screening] Visco Rsq column not found; tau_Visco Rsq screening skipped.")
...
def prepare_ni_raw_line_data(
    ...
    """
    Prepare raw point-level NI data for regression plots.

    Uses:
        x = normalised point position through dermis
        y = raw variable value

    The normalised position is calculated per sample from point order:
        0–100% through the dermis.
    """
    ...
    # Sort through the dermis by y then x, matching the original layer assignment logic.
    sort_cols = ["Sample"]
    if "SampleKey" in d.columns:
        sort_cols = ["SampleKey"]

    d = d.sort_values(sort_cols + ["y", "x"]).copy()

    # Create point index and normalised position per sample.
    sample_col = "SampleKey" if "SampleKey" in d.columns else "Sample"
...
def prepare_ni_pca_data(
    ...
    scores_df : pd.DataFrame
        Original cleaned NI rows with PCA scores added.

    loadings_df : pd.DataFrame
        PCA loadings in tidy long format:
            PC, Variable, Loading, AbsLoading, ExplainedVarianceRatio,
            ExplainedVariancePercent

    pca_info : dict
        PCA metadata including:
            pca_vars, pca_cols, standardised, mu, sd, loadings matrix,
    ...
    d = d[
        d["SubtypeClean"].astype(str).isin(wanted_subtypes)
    ].copy()
    ...
    if d.empty:
        ...
        available = prepare_ni_raw_line_data(
            ni_raw=ni_raw,
            variable=base_var,
            subtype_order=subtype_order,
            convert_modulus_to_kpa=False,
    ...
    # Keep PC1/PC2 present even if not enough components
    if "PC1" not in scores_df.columns:
        scores_df["PC1"] = np.nan

    if "PC2" not in scores_df.columns:
        scores_df["PC2"] = np.nan

    # Add explained variance columns for convenience
    for i, frac in enumerate(evr, start=1):
        scores_df[f"PC{i}_ExplainedVarianceRatio"] = frac
        scores_df[f"PC{i}_ExplainedVariancePercent"] = frac * 100
    ...
    for pc_idx in range(loadings.shape[1]):
        ...
        for var_name, loading_value in zip(pca_cols, loadings[:, pc_idx]):
            ...
            loading_rows.append(
                {
                    ...
                    "AbsLoading": abs(loading_value),
                    "ExplainedVarianceRatio": explained_ratio,
                    "ExplainedVariancePercent": explained_ratio * 100,
                    "Standardised": standardise,
                    "MeanUsedForScaling": mu[pca_cols.index(var_name)],
                    "SDUsedForScaling": sd[pca_cols.index(var_name)],
                }
            )
    ...
    explained_df = pd.DataFrame(
        {
            ...
            "ExplainedVariancePercent": evr * 100,
        }
    )
    ...
    return scores_df, loadings_df, pca_info
...
def plot_cell_wound_vs_dermis_shift(
    ...
    errorbar="sem",   # "sem" or "std"
):
    """
    Plot fibroblast density % shift from dermis to wound.

    Bars:
        lower_shift and upper_shift within each subtype.

    Error bars:
        SEM or STD, controlled by errorbar.

    Statistics:
        paired t-test between lower_shift and upper_shift within each subtype.

    Annotation:
        adds a horizontal bracket and star above significant lower/upper pairs.
    """

    subtypes = resolve_subtypes_to_plot(
        subtype_order,
        subtypes_to_plot,
    )
    ...
    shift_df = shift_df[
        shift_df["SubtypeClean"].astype(str).isin(subtypes)
    ].copy()

    summary = summary[
        summary["SubtypeClean"].astype(str).isin(subtypes)
    ].copy()

    if shift_df.empty or summary.empty:
        print("No cell shift data available.")
        return None, None, shift_df, summary, pd.DataFrame()

    # ------------------------------------------------------------
    # Decide error bar column
    # ------------------------------------------------------------
    if errorbar.lower() == "std" and "std" in summary.columns:
        err_col = "std"
    elif errorbar.lower() == "sem" and "sem" in summary.columns:
        err_col = "sem"
    elif "err" in summary.columns:
        err_col = "err"
    elif "sem" in summary.columns:
        err_col = "sem"
    ...
    x_centres = np.arange(len(subtypes), dtype=float)

    bar_group_width = 0.78
    n_bars = len(shift_order)
    bar_width = bar_group_width / n_bars

    alpha_start, alpha_end = 0.95, 0.35
    shift_alphas = np.linspace(alpha_start, alpha_end, len(shift_order))
    ...
    try:
        ...
        for j, shift_type in enumerate(shift_order):
            ...
            for subtype in subtypes:
                ...
                row = summary[
                    (summary["SubtypeClean"].astype(str) == str(subtype))
                    & (summary["ShiftType"].astype(str) == shift_type)
                ]
                ...
                if row.empty:
                    ...
                    if err_col is not None and pd.notna(row[err_col].iloc[0]):
            ...
            # Hatch overlay, matching grouped cell bar style
            if hatches:
                for bar, subtype in zip(bars, subtypes):
                    ...
                    ax.bar(
                        ...
                        edgecolor=hatch_colour,
                        linewidth=0.0,
                        hatch=region_hatches.get(shift_type, ""),
                        alpha=1.0,
                        zorder=3,
                    )
            ...
            for subtype, xpos, mean, err in zip(subtypes, xs, means, errs):
                bar_positions[(str(subtype), shift_type)] = {
                    "x": xpos,
                    "mean": mean,
                    "err": err,
                }
    ...
    ax.set_ylabel("% shift from dermis")
    ax.set_title("Fibroblast density % shift from dermis to wound")
    ax.grid(False)
    ...
    if run_stats:
        ...
        for c in [
            ...
            if c in shift_df.columns:
                sample_col = c
                break

        if sample_col is None:
            raise KeyError(
                "Could not find sample/pairing column in shift_df. "
                f"Available columns are: {list(shift_df.columns)}"
            )
        ...
        if value_col is None:
            raise KeyError(
                "Could not find shift value column in shift_df. "
                f"Available columns are: {list(shift_df.columns)}"
            )
        ...
        for subtype in subtypes:
            ...
            d_sub = shift_df[
                shift_df["SubtypeClean"].astype(str) == str(subtype)
            ].copy()

            if d_sub.empty:
                continue

            d_sub[value_col] = pd.to_numeric(
                d_sub[value_col],
                errors="coerce",
            )
            ...
            if (
                "lower_shift" not in wide.columns
                or "upper_shift" not in wide.columns
            ):
                continue
            ...
            if n_pairs < 2:
                ...
                lower_vals = wide["lower_shift"].to_numpy(float)
                upper_vals = wide["upper_shift"].to_numpy(float)
                ...
                try:
                    ...
                    t_stat, p_value = np.nan, np.nan
                    note = f"paired t-test failed: {type(exc).__name__}: {exc}"

            rows.append(
                {
                    "ComparisonType": "within-subtype paired t-test",
                    "Subtype": subtype,
                    "Label": NI_LABELS.get(subtype, subtype),
                    "Comparison": "lower_shift vs upper_shift",
                    "SelectedTest": "paired t-test",
                    "n_pairs": n_pairs,
                    "MeanLower": (
                        float(wide["lower_shift"].mean())
                        if n_pairs > 0
                        else np.nan
                    ),
                    "MeanUpper": (
                        float(wide["upper_shift"].mean())
                        if n_pairs > 0
                        else np.nan
                    ),
                    "MeanDifference_Upper_minus_Lower": (
                        float((wide["upper_shift"] - wide["lower_shift"]).mean())
                        if n_pairs > 0
                        else np.nan
                    ),
                    "TestNote": note,
        ...
        if print_stats:
            print("\n[Cell matched lower vs upper % shift paired t-tests]")
            if stats_df.empty:
                print("No paired t-tests were run.")
            else:
                print(
                    stats_df[
                        [
    ...
    # ------------------------------------------------------------
    if annotate_stats and not stats_df.empty:
        ...
        for _, row in stats_df.iterrows():
            ...
            if (
                (subtype, "lower_shift") not in bar_positions
                or (subtype, "upper_shift") not in bar_positions
            ):
                continue

            lower_bar = bar_positions[(subtype, "lower_shift")]
            upper_bar = bar_positions[(subtype, "upper_shift")]

            x1 = lower_bar["x"]
            x2 = upper_bar["x"]

            y_top = max(
                lower_bar["mean"] + lower_bar["err"],
                upper_bar["mean"] + upper_bar["err"],
            )

            y_line = y_top + 0.08 * y_range
            h = 0.025 * y_range
            ...
            y_needed = max(
                y_needed,
                y_line + h + 0.12 * y_range,
            )

        if y_needed > current_ymax:
            ax.set_ylim(
                current_ymin,
                y_needed,
            )
    ...
    return fig, ax, shift_df, summary, stats_df
...
def calculate_cell_dermis_to_wound_percent_shift(
    cell_raw,
    *,
    value_col=CELL_VALUE_COL,
    subtype_order=SUBTYPE_ORDER,
    ...
    """
    Calculate pooled % shift from dermis to wound for fibroblast density.
    ...
    Pairing:
        Pairs dermis and wound values by order within each subtype,
        matching the logic used in calculate_cell_wound_vs_dermis_shift().

    For each pair:
        % shift = ((wound - dermis) / dermis) * 100
    """
    ...
    d = prepare_cell_dermis_wound_pooled(
        cell_raw,
        value_col=value_col,
    )
    ...
    d = d[
        d["SubtypeClean"].astype(str).isin(subtypes)
    ].copy()

    if d.empty:
        print("No matching cell rows for dermis-to-wound shift.")
        return pd.DataFrame(), pd.DataFrame()
    ...
    for subtype, d_sub in d.groupby("SubtypeClean", observed=True):
        ...
        dermis = (
            d_sub[d_sub["Region"].astype(str) == "dermis"]
            [[value_col]]
            .dropna()
        ...
        wound = (
            d_sub[d_sub["Region"].astype(str) == "wound"]
            [[value_col]]
            .dropna()
            .reset_index(drop=True)
        )
        ...
        for i in range(n):
            ...
            baseline = float(
                dermis[value_col].iloc[i]
            )

            target = float(
                wound[value_col].iloc[i]
            )
...
def plot_cell_dermis_to_wound_percent_shift_anova(
    cell_raw,
    *,
    value_col=CELL_VALUE_COL,
    subtype_order=SUBTYPE_ORDER,
    ...
    colours=NI_COLOURS,
    labels=NI_LABELS,
    ylabel="% shift from dermis to wound",
    ...
    """
    Plot pooled dermis-to-wound % shift in fibroblast density.

    Bars:
        mean % shift per subtype

    Error bars:
        SEM or SD, controlled by errorbar

    Statistics:
        1. Overall one-way ANOVA across all subtype bars.
        2. Pairwise one-way ANOVA between every subtype pair.

    Scatter points are intentionally not plotted.
    """
    ...
    # Control usually has no wound region, so exclude it from dermis-to-wound shift.
    subtypes = [
        st for st in subtypes
        if str(st) != "control"
    ]

    shift_df, summary = calculate_cell_dermis_to_wound_percent_shift(
        cell_raw,
        value_col=value_col,
        subtype_order=subtype_order,
        subtypes_to_plot=subtypes,
        exclude_control=True,
    )

    if shift_df.empty or summary.empty:
        print("No dermis-to-wound shift data to plot.")
        return None, None, shift_df, summary, pd.DataFrame(), pd.DataFrame()
    ...
    if not present_subtypes:
        print("No matching subtypes after shift calculation.")
        return None, None, shift_df, summary, pd.DataFrame(), pd.DataFrame()

    summary["err"] = (
        summary["std"]
        if errorbar.lower() == "std"
        else summary["sem"]
    )
    ...
    if run_stats:
        ...
        if print_stats:
            print("\n[Cell dermis-to-wound % shift values going into ANOVA]")
        ...
        if len(groups) >= 2:
            ...
            try:
                ...
                stat, p_value = np.nan, np.nan
                anova_note = f"ANOVA failed: {type(exc).__name__}: {exc}"

            overall_stats_df = pd.DataFrame(
                [
                    {
                        "ComparisonType": "overall between-subtype one-way ANOVA",
                        "ShiftType": "wound_vs_dermis",
                        "Comparison": " vs ".join(
                            labels.get(st, st)
                            for st in group_names
                        ...
                        "SelectedTest": "one-way ANOVA",
                        "GroupNs": ", ".join(
                            f"{labels.get(st, st)}: n={len(groups[st])}"
                            for st in group_names
                        ),
            ...
            if add_stats_text:
                if np.isfinite(p_value):
                    stat_text = f"Overall ANOVA p = {p_value:.3g} ({p_to_stars(p_value)})"
                else:
                    stat_text = "Overall ANOVA p = n/a"
        ...
        # --------------------------------------------------------
        # Pairwise one-way ANOVAs between every subtype pair
        # --------------------------------------------------------
        pairwise_rows = []

        for subtype_a, subtype_b in combinations(groups.keys(), 2):

            vals_a = groups[subtype_a]
            vals_b = groups[subtype_b]

            if len(vals_a) < 2 or len(vals_b) < 2:
                stat, p_value = np.nan, np.nan
                note = "not enough values"
            else:
                try:
                    stat, p_value = stats.f_oneway(vals_a, vals_b)
                    note = "OK"
                except Exception as exc:
                    stat, p_value = np.nan, np.nan
                    note = f"Pairwise ANOVA failed: {type(exc).__name__}: {exc}"

            pairwise_rows.append(
                {
                    "ComparisonType": "pairwise between-subtype one-way ANOVA",
                    "ShiftType": "wound_vs_dermis",
                    "SubtypeA": subtype_a,
                    "SubtypeB": subtype_b,
                    "Comparison": f"{labels.get(subtype_a, subtype_a)} vs {labels.get(subtype_b, subtype_b)}",
                    "SelectedTest": "one-way ANOVA",
                    "n_A": len(vals_a),
                    "n_B": len(vals_b),
        ...
        # --------------------------------------------------------
        if annotate_pairwise and not pairwise_stats_df.empty:
            ...
            annotation_rows = pairwise_stats_df.copy()

            if annotate_only_significant:
                annotation_rows = annotation_rows[
                    annotation_rows["p_value"].notna()
                    & (annotation_rows["p_value"] < 0.05)
                ].copy()

            annotation_rows = annotation_rows.reset_index(drop=True)
            ...
            if not annotation_rows.empty:
                ...
                y_base = np.nanmax(y + np.nan_to_num(yerr, nan=0.0))
                step = 0.08 * y_range

                for k, row in annotation_rows.iterrows():

                    subtype_a = row["SubtypeA"]
                    subtype_b = row["SubtypeB"]

                    if subtype_a not in subtype_to_x or subtype_b not in subtype_to_x:
                        continue

                    x1 = subtype_to_x[subtype_a]
                    x2 = subtype_to_x[subtype_b]
                ...
                ax.set_ylim(
                    current_ymin,
                    y_base + step * (len(annotation_rows) + 2),
                )

        if print_stats:

            print("\n[Cell dermis-to-wound % shift overall ANOVA]")
            if overall_stats_df.empty:
                print("No overall ANOVA test was run.")
            else:
                print(overall_stats_df.to_string(index=False))

            print("\n[Cell dermis-to-wound % shift pairwise ANOVAs]")
            if pairwise_stats_df.empty:
                print("No pairwise tests were run.")
            else:
    ...
    return fig, ax, shift_df, summary, overall_stats_df, pairwise_stats_df
...
def plot_saxs_total_collagen_dermis_vs_wound(
    ...
    Uses point-level data, normalises first, then computes sample means.

    paired_test=True:
        Performs paired t-test between dermis and wound within each subtype,
        using sample-level paired dermis/wound means.
    """
    ...
    fig, ax = _plot_saxs_grouped_bars(
        summary=summary,
        points=sample_means.rename(columns={"SampleMean": plot_value_col}),
        regions=["dermis", "wound"],
        ...
        ylim=ylim,
        hatches=hatches,
        show=False,   # important: delay showing until after annotations
    )

    stats_df = pd.DataFrame()

    if paired_test and fig is not None and ax is not None:
        stats_df = paired_region_t_tests(
            sample_means,
        ...
        if print_stats and not stats_df.empty:
            ...
            for c in ["MeanA", "MeanB", "MeanDifference_B_minus_A", "t_stat", "p_value"]:
                stats_print[c] = pd.to_numeric(stats_print[c], errors="coerce")

            print("\n[SAXS paired t-tests: dermis vs wound]")
            print(
                stats_print[
                    [
                        "Label",
...
def plot_saxs_total_collagen_4region(
    ...
    return _plot_saxs_grouped_bars(
        ...
        # title="SAXS total collagen intensity by spatial region",
        title=" ",
        ylabel=ylabel,
        region_col_points="Region",
        value_col=plot_value_col,
...
def plot_saxs_dperiod_shift_dermis_to_wound(
    ...
    Statistics:
        1. Overall one-way ANOVA across post-wounding subtype bars.
        2. Pairwise one-way ANOVA between every subtype pair.

    Stats are run on sample-level % shift values from shift_df.
    """
    ...
    if shift_df is None or shift_df.empty:
        print("No SAXS D-period shift values available for statistics.")
        return fig, ax, shift_df, summary, overall_stats_df, pairwise_stats_df

    d = shift_df.copy()

    # ------------------------------------------------------------
    # Identify the % shift column
    # ------------------------------------------------------------
    possible_value_cols = [
        "PercentShift",
        "percent_shift",
    ...
    if value_col is None:
        raise KeyError(
            "Could not find the % shift value column in shift_df. "
            f"Available columns are: {list(d.columns)}"
        )

    # ------------------------------------------------------------
    # Filter to the relevant shift type if present
    # ------------------------------------------------------------
    if "ShiftType" in d.columns:
        d = d[
            d["ShiftType"].astype(str) == "wound_vs_dermis"
        ].copy()
    ...
    # Exclude control from stats, because there is no wound region.
    d = d[
        d["SubtypeClean"].astype(str) != "control"
    ].copy()

    present_subtypes = [
        st for st in subtype_order
        if str(st) != "control"
        and str(st) in set(d["SubtypeClean"].astype(str))
    ]

    if not present_subtypes:
        print("No post-wounding subtype values available for statistics.")
        return fig, ax, shift_df, summary, overall_stats_df, pairwise_stats_df
    ...
    for subtype in present_subtypes:
        ...
        vals = (
            d.loc[
                d["SubtypeClean"].astype(str) == str(subtype),
                value_col,
            ]
            .dropna()
            .to_numpy(dtype=float)
    ...
    if print_stats:
        print("\n[SAXS D-period dermis-to-wound % shift values going into ANOVA]")
        if group_summary_df.empty:
            print("No valid groups.")
        else:
            print(group_summary_df.to_string(index=False))
            ...
            [
                {
                    ...
                    "SelectedTest": "one-way ANOVA",
                    "GroupNs": ", ".join(
                        f"{NI_LABELS.get(st, st)}: n={len(groups[st])}"
                        for st in group_names
                    ),
                    "ANOVA_Note": anova_note,
                    "statistic": stat,
        ...
        pairwise_rows.append(
            {
                ...
                "SubtypeA": subtype_a,
                "SubtypeB": subtype_b,
                "Comparison": f"{NI_LABELS.get(subtype_a, subtype_a)} vs {NI_LABELS.get(subtype_b, subtype_b)}",
                "SelectedTest": "one-way ANOVA",
                "n_A": len(vals_a),
                "n_B": len(vals_b),
                "Mean_A": float(np.mean(vals_a)) if len(vals_a) else np.nan,
    ...
    if print_stats:
        ...
        print("\n[SAXS D-period dermis-to-wound % shift overall ANOVA]")
        if overall_stats_df.empty:
            print("No overall ANOVA test was run.")
        else:
            print(overall_stats_df.to_string(index=False))

        print("\n[SAXS D-period dermis-to-wound % shift pairwise ANOVAs]")
        if pairwise_stats_df.empty:
            print("No pairwise tests were run.")
        else:
            print(pairwise_stats_df.to_string(index=False))
...
def plot_saxs_dperiod_fwhm_dermis_vs_wound(
    ...
    return _plot_saxs_grouped_bars(
        summary=summary,
        points=spread_samples.rename(columns={"FWHM": value_col}),
        regions=["dermis", "wound"],
        # title="D-period spread (FWHM) in dermis and wound",
        title="",
...
def plot_saxs_peak_position_bar(
    ...
    Uses point-level folded peak positions -> sample means -> subtype mean ± SD.
    """
    ...
    d = pool_saxs_dermis_wound_regions(d)
    d = d.rename(columns={"RegionPooled": "RegionForPlot"})
    d = d[d["RegionForPlot"].isin(regions)].copy()
    ...
    return _plot_saxs_grouped_bars(
        summary=summary,
        points=sample_means.rename(columns={"SampleMean": peak_param}),
...
def plot_saxs_peak_position_shift_from_dermis(
    ...
    """
    SAXS orientation peak-position shift from dermis.
    ...
    Stats are run on sample-level shift values from shift_df.
    Control/Unwounded is excluded from the ANOVA if present.
    """
    ...
    fig, ax = _plot_saxs_shift_bars(
        ...
        shift_order=["wound_vs_dermis"],
        title="",
        ylabel="Peak position shift from dermis (°)",
        subtype_order=subtype_order,
        ylim=ylim,
        hatches=hatches,
        keep_empty_subtypes=keep_empty_subtypes,
    ...
    if shift_df is None or shift_df.empty:
        print("No SAXS peak-position shift values available for statistics.")
        return fig, ax, shift_df, summary, overall_stats_df, pairwise_stats_df
    ...
    # ------------------------------------------------------------
    # Filter to wound_vs_dermis if available
    # ------------------------------------------------------------
    if "ShiftType" in d.columns:
        d = d[
            d["ShiftType"].astype(str) == "wound_vs_dermis"
    ...
    # Exclude control from stats, because the test is between post-wounding bars.
    d = d[
        d["SubtypeClean"].astype(str) != "control"
    ].copy()
    ...
    if print_stats:
        print("\n[SAXS peak-position shift values going into ANOVA]")
        if group_summary_df.empty:
            print("No valid groups.")
        else:
            print(group_summary_df.to_string(index=False))
    ...
    if print_stats:
        ...
        print("\n[SAXS peak-position shift overall ANOVA]")
        if overall_stats_df.empty:
            print("No overall ANOVA test was run.")
        else:
            print(overall_stats_df.to_string(index=False))

        print("\n[SAXS peak-position shift pairwise ANOVAs]")
        if pairwise_stats_df.empty:
            print("No pairwise tests were run.")
        else:
            print(pairwise_stats_df.to_string(index=False))
...
def plot_saxs_peak_spread_dermis_vs_wound(
    ...
    return _plot_saxs_peak_spread_grouped_bars(
...
def calculate_saxs_dermis_to_wound_percent_shift(
    saxs_points,
    *,
    parameter="curvearea",
    normalise=True,
    ...
    """
    Calculate % shift in SAXS signal from dermis to wound.

    Uses point-level SAXS data, applies the usual SAXS preparation/
    normalisation, then pools regions into:

        dermis = dermis_sub + dermis_epi
        wound  = wound_sub + wound_epi

    For each subtype and sample:
        dermis mean = mean normalised SAXS value in dermis
        wound mean  = mean normalised SAXS value in wound
    ...
    shift_df : pd.DataFrame
        One row per sample/subtype with dermis mean, wound mean and % shift.

    shift_summary : pd.DataFrame
        Mean, SD, SEM and n per subtype.
    """
    ...
    if d.empty:
        print("No matching SAXS subtypes available for dermis-to-wound shift.")
        return pd.DataFrame(), pd.DataFrame()

    # ------------------------------------------------------------
    # Sample-level means per pooled region
    # ------------------------------------------------------------
    sample_means, summary_unused = summarise_saxs_points_to_sample_bars(
        d,
        value_col=plot_value_col,
        region_col="RegionForPlot",
    )
    ...
    if sample_means.empty:
        print("No sample-level SAXS means available.")
        return pd.DataFrame(), pd.DataFrame()
    ...
    for c in [
        ...
        if c in sample_means.columns:
            sample_col = c
            break

    if sample_col is None:
        raise KeyError(
            "Could not find sample column in SAXS sample means. "
            f"Available columns are: {list(sample_means.columns)}"
        )

    if "Region" in sample_means.columns:
        region_col = "Region"
    elif "RegionForPlot" in sample_means.columns:
        region_col = "RegionForPlot"
    else:
        raise KeyError(
            "Could not find region column in SAXS sample means. "
            f"Available columns are: {list(sample_means.columns)}"
        )

    sample_means["SampleMean"] = pd.to_numeric(
        sample_means["SampleMean"],
        errors="coerce",
    )
    ...
    if "dermis" not in wide.columns or "wound" not in wide.columns:
        print("Could not find both dermis and wound columns.")
    ...
    if wide.empty:
        print("No paired dermis/wound SAXS sample means available.")
        return pd.DataFrame(), pd.DataFrame()

    wide["PercentShift"] = (
        (wide["wound"] - wide["dermis"])
        / wide["dermis"]
    ) * 100
    ...
    wide["ShiftType"] = "wound_vs_dermis"
    wide["Parameter"] = parameter
    wide["Normalised"] = normalise
    ...
    shift_summary["sem"] = (
        shift_summary["std"]
        / np.sqrt(shift_summary["n"].clip(lower=1))
    )

    return shift_df, shift_summary


def plot_saxs_dermis_to_wound_percent_shift_anova(
    saxs_points,
    *,
    parameter="curvearea",
    normalise=True,
    ...
    """
    Plot % shift in SAXS signal from dermis to wound.
    ...
    # Control usually has no wound, so exclude it from dermis-to-wound shift.
    subtypes = [
        st for st in subtypes
        if str(st) != "control"
    ]

    shift_df, summary = calculate_saxs_dermis_to_wound_percent_shift(
        saxs_points,
        parameter=parameter,
        normalise=normalise,
        subtype_order=subtype_order,
    ...
    if shift_df.empty or summary.empty:
        print("No SAXS dermis-to-wound shift data to plot.")
        return None, None, shift_df, summary, pd.DataFrame(), pd.DataFrame()
    ...
    if not present_subtypes:
        print("No matching subtypes after SAXS shift calculation.")
        return None, None, shift_df, summary, pd.DataFrame(), pd.DataFrame()
    ...
    if ylabel is None:
        if normalise:
            ylabel = "% shift in normalised collagen intensity\nfrom dermis to wound"
        else:
            ylabel = f"% shift in {SAXS_PARAMETER_LABELS.get(parameter, parameter)}\nfrom dermis to wound"
    ...
    if run_stats:
        ...
        if print_stats:
            print("\n[SAXS dermis-to-wound % shift values going into ANOVA]")
            if group_summary_df.empty:
                print("No valid groups.")
            else:
                print(group_summary_df.to_string(index=False))
        ...
        if print_stats:
            ...
            print("\n[SAXS dermis-to-wound % shift overall ANOVA]")
            if overall_stats_df.empty:
                print("No overall ANOVA test was run.")
            else:
                print(overall_stats_df.to_string(index=False))

            print("\n[SAXS dermis-to-wound % shift pairwise ANOVAs]")
            if pairwise_stats_df.empty:
                print("No pairwise tests were run.")
            else:
                print(pairwise_stats_df.to_string(index=False))
...
def get_unwounded_dperiod_fwhm_reference_error(
    saxs_points,
    *,
    trim_std_devs=6,
    ...
    """
    Get unwounded dermal D-period FWHM reference error for a % shift plot.

    Since Unwounded has no wound region, it is plotted at 0%.
    The error bar is shown as relative SEM:

        relative SEM (%) = (SEM dermal FWHM / mean dermal FWHM) * 100
    """

    spread_samples, summary_unused, value_col = prepare_saxs_dperiod_fwhm_from_spread(
        saxs_points=saxs_points,
        regions=["dermis"],
        pool_to_dermis_wound=True,
        trim_std_devs=trim_std_devs,
    ...
    d = d[
        (d["SubtypeClean"].astype(str) == str(control_label))
        & (d["Region"].astype(str) == "dermis")
    ].copy()
    ...
    if vals.size > 1:
        sem_val = float(np.std(vals, ddof=1) / np.sqrt(vals.size))
    else:
        sem_val = np.nan
    ...
    if not np.isfinite(mean_val) or mean_val == 0:
        ...
        relative_sem_percent = (sem_val / mean_val) * 100

    return 0.0, relative_sem_percent, int(vals.size)

def calculate_saxs_dperiod_fwhm_dermis_to_wound_percent_shift(
    saxs_points,
    *,
    subtype_order=SUBTYPE_ORDER,
    subtypes_to_plot=None,
    ...
    """
    Calculate % shift in D-period FWHM from dermis to wound.

    FWHM is calculated upstream by prepare_saxs_dperiod_fwhm_from_spread():

        FWHM = 2.3548 * within-sample SD of raw D-period values

    For each subtype and sample:
        % shift = ((wound FWHM - dermis FWHM) / dermis FWHM) * 100
    """
    ...
    if spread_samples.empty:
        print("No SAXS D-period FWHM sample data available.")
        return pd.DataFrame(), pd.DataFrame()
    ...
    if d.empty:
        print("No matching SAXS subtypes available for D-period FWHM shift.")
        return pd.DataFrame(), pd.DataFrame()
    ...
    if sample_col is None:
        raise KeyError(
            "Could not find sample column in SAXS FWHM data. "
            f"Available columns are: {list(d.columns)}"
        )

    if "Region" not in d.columns:
        raise KeyError(
            "Could not find Region column in SAXS FWHM data. "
            f"Available columns are: {list(d.columns)}"
        )
    ...
    if value_col not in d.columns:
        if "FWHM" in d.columns:
            ...
            raise KeyError(
                f"Could not find FWHM value column '{value_col}'. "
                f"Available columns are: {list(d.columns)}"
            )
    ...
    if "dermis" not in wide.columns or "wound" not in wide.columns:
        print("Could not find both dermis and wound FWHM columns.")
        print("Available columns:")
        print(wide.columns.tolist())
        return pd.DataFrame(), pd.DataFrame()
    ...
    if wide.empty:
        print("No paired dermis/wound SAXS D-period FWHM values available.")
        return pd.DataFrame(), pd.DataFrame()
...
def plot_saxs_dperiod_fwhm_dermis_to_wound_percent_shift_anova(
    saxs_points,
    *,
    subtype_order=SUBTYPE_ORDER,
    subtypes_to_plot=None,
    ...
    annotate_pairwise=True,
    annotate_only_significant=True,
    include_unwounded_gap=True,
):
    """
    Plot % shift in D-period FWHM from dermis to wound.
    ...
    Unwounded:
        shown as a 0% reference slot because there is no wound region.
        Its error bar represents the relative SEM of unwounded dermal FWHM.

    Statistics:
        1. Overall one-way ANOVA across post-wounding subtype bars only.
        2. Pairwise one-way ANOVA between every post-wounding subtype pair.

    No title is added.
    X tick labels are rotation=0.
    """
    ...
    # Control usually has no wound region, so exclude it from the shift statistics.
    subtypes = [
        st for st in subtypes
        if str(st) != "control"
    ]

    shift_df, summary = calculate_saxs_dperiod_fwhm_dermis_to_wound_percent_shift(
        saxs_points=saxs_points,
        subtype_order=subtype_order,
        subtypes_to_plot=subtypes,
        trim_std_devs=trim_std_devs,
    ...
    if shift_df.empty or summary.empty:
        print("No SAXS D-period FWHM dermis-to-wound shift data to plot.")
        return None, None, shift_df, summary, pd.DataFrame(), pd.DataFrame()
    ...
    if not present_subtypes:
        print("No matching subtypes after SAXS FWHM shift calculation.")
        return None, None, shift_df, summary, pd.DataFrame(), pd.DataFrame()
    ...
    # ------------------------------------------------------------
    # X positions
    # Unwounded shown at x=0, then a gap, then PW bars.
    # ------------------------------------------------------------
    if include_unwounded_gap:
        control_x = 0
        x = np.arange(len(present_subtypes)) + 2
    else:
        control_x = None
        x = np.arange(len(present_subtypes))
    ...
    # ------------------------------------------------------------
    # Add Unwounded 0% reference slot
    # ------------------------------------------------------------
    if include_unwounded_gap:

        control_y, control_yerr, control_n = get_unwounded_dperiod_fwhm_reference_error(
            saxs_points=saxs_points,
            trim_std_devs=trim_std_devs,
            control_label="control",
        )
        ...
        ax.bar(
            control_x,
            0,
            color=colours.get("control", "white"),
            edgecolor="black",
            linewidth=0.8,
            alpha=0.25,
            zorder=2,
    ...
    if run_stats:
        ...
        if print_stats:
            print("\n[SAXS D-period FWHM dermis-to-wound % shift values going into ANOVA]")
            if group_summary_df.empty:
                print("No valid groups.")
            else:
                print(group_summary_df.to_string(index=False))
        ...
        if annotate_pairwise and not pairwise_stats_df.empty:
            ...
            if not annotation_rows.empty:
                ...
                subtype_to_x = {
                    subtype: xpos
                    for subtype, xpos in zip(present_subtypes, x)
                }
        ...
        if print_stats:
            ...
            print("\n[SAXS D-period FWHM dermis-to-wound % shift overall ANOVA]")
            if overall_stats_df.empty:
                print("No overall ANOVA test was run.")
            else:
                print(overall_stats_df.to_string(index=False))

            print("\n[SAXS D-period FWHM dermis-to-wound % shift pairwise ANOVAs]")
            if pairwise_stats_df.empty:
                print("No pairwise tests were run.")
            else:
                print(pairwise_stats_df.to_string(index=False))
...
def calculate_saxs_total_collagen_4region_matched_percent_shift(
    saxs_points,
    *,
    parameter="curvearea",
    normalise=True,
    ...
    subtypes_to_plot=None,
    trim_std_devs=6,
    include_control_reference=True,
):
    """
    Calculate matched % shift in SAXS collagen intensity:

        lower shift = lower wound vs lower dermis
        upper shift = upper wound vs upper dermis

    For each subtype/sample:

        lower_shift = ((wound_sub - dermis_sub) / dermis_sub) * 100
        upper_shift = ((wound_epi - dermis_epi) / dermis_epi) * 100

    Error bars are calculated later as STD.
    """
    ...
    if d.empty:
        print("No matching SAXS subtypes available.")
        return pd.DataFrame(), pd.DataFrame()
    ...
    if sample_col is None:
        raise KeyError(
            "Could not find sample column in SAXS data. "
            f"Available columns are: {list(d.columns)}"
        )
    ...
    if region_col is None:
        raise KeyError(
            "Could not find region column in SAXS data. "
            f"Available columns are: {list(d.columns)}"
        )

    d[plot_value_col] = pd.to_numeric(
        d[plot_value_col],
        errors="coerce",
    )
    ...
    # ------------------------------------------------------------
    # Standardise possible region names
    # ------------------------------------------------------------
    region_map = {
        "lower_dermis": "dermis_sub",
        "upper_dermis": "dermis_epi",
        "lower wound": "wound_sub",
        "upper wound": "wound_epi",
        "lower_wound": "wound_sub",
        "upper_wound": "wound_epi",
        "lower wound bed": "wound_sub",
        "upper wound bed": "wound_epi",
        "lower dermis": "dermis_sub",
        "upper dermis": "dermis_epi",
        "dermis_sub": "dermis_sub",
        "dermis_epi": "dermis_epi",
        "wound_sub": "wound_sub",
        "wound_epi": "wound_epi",
    }
    ...
    # If mapping failed, keep original value
    d["_RegionForShift"] = d["_RegionForShift"].fillna(
        d[region_col].astype(str).str.strip()
    )

    # ------------------------------------------------------------
    # Sample-level means for each exact region
    # ------------------------------------------------------------
    sample_region_means = (
        d.groupby(
            [
        ...
        .mean()
        .rename(columns={plot_value_col: "SampleMean"})
    )
    ...
    for _, row in wide.iterrows():
        ...
        # lower wound vs lower dermis
        if (
            "dermis_sub" in wide.columns
            and "wound_sub" in wide.columns
            and pd.notna(row.get("dermis_sub"))
            and pd.notna(row.get("wound_sub"))
            and np.isfinite(row.get("dermis_sub"))
            and np.isfinite(row.get("wound_sub"))
            and row.get("dermis_sub") != 0
        ):
            lower_shift = (
                (row["wound_sub"] - row["dermis_sub"])
                / row["dermis_sub"]
            ) * 100
            ...
            rows.append(
                {
                    "SubtypeClean": subtype,
                    "Sample": sample,
                    "ShiftType": "lower_shift",
                    "ShiftLabel": "Lower wound vs lower dermis",
                    "BaselineRegion": "dermis_sub",
                    "TargetRegion": "wound_sub",
                    "BaselineValue": row["dermis_sub"],
                    "TargetValue": row["wound_sub"],
                    "PercentShift": lower_shift,
        ...
        # upper wound vs upper dermis
        if (
            "dermis_epi" in wide.columns
            and "wound_epi" in wide.columns
            and pd.notna(row.get("dermis_epi"))
            and pd.notna(row.get("wound_epi"))
            and np.isfinite(row.get("dermis_epi"))
            and np.isfinite(row.get("wound_epi"))
            and row.get("dermis_epi") != 0
        ):
            upper_shift = (
                (row["wound_epi"] - row["dermis_epi"])
                / row["dermis_epi"]
            ) * 100
            ...
            rows.append(
                {
                    "SubtypeClean": subtype,
                    "Sample": sample,
                    "ShiftType": "upper_shift",
                    "ShiftLabel": "Upper wound vs upper dermis",
                    "BaselineRegion": "dermis_epi",
                    "TargetRegion": "wound_epi",
                    "BaselineValue": row["dermis_epi"],
                    "TargetValue": row["wound_epi"],
                    "PercentShift": upper_shift,
                    "IsControlReference": False,
                }
            )
    ...
    # ------------------------------------------------------------
    # Add Unwounded reference rows at 0%
    # ------------------------------------------------------------
    if include_control_reference and "control" in set(wide["SubtypeClean"].astype(str)):

        control_rows = wide[
            wide["SubtypeClean"].astype(str) == "control"
        ].copy()

        for shift_type, dermis_region, label in [
            ("lower_shift", "dermis_sub", "Lower dermis reference"),
            ("upper_shift", "dermis_epi", "Upper dermis reference"),
        ]:

            if dermis_region not in control_rows.columns:
                continue

            vals = pd.to_numeric(
                control_rows[dermis_region],
                errors="coerce",
            ).dropna().to_numpy(float)
            ...
            if np.isfinite(mean_val) and mean_val != 0 and np.isfinite(std_val):
                relative_std = (std_val / mean_val) * 100
            else:
                relative_std = np.nan

            for sample in control_rows[sample_col].to_list():
                shift_df = pd.concat(
                    [
                        shift_df,
                        pd.DataFrame(
                            [
                                {
                                    "SubtypeClean": "control",
                                    "Sample": sample,
                                    "ShiftType": shift_type,
                                    "ShiftLabel": label,
                                    "BaselineRegion": dermis_region,
                                    "TargetRegion": "none",
                                    "BaselineValue": np.nan,
                                    "TargetValue": np.nan,
                                    "PercentShift": 0.0,
                                    "ControlRelativeSTD": relative_std,
    ...
    if shift_df.empty:
        print("No matched SAXS 4-region shift values calculated.")
        print("Available region names were:")
        print(sorted(d[region_col].dropna().astype(str).unique()))
        return shift_df, pd.DataFrame()

    # ------------------------------------------------------------
    # Summary: STD error bars
    # ------------------------------------------------------------
    summary = (
        shift_df
        .groupby(
    ...
    # Replace control std with relative STD of dermal reference if available
    if "ControlRelativeSTD" in shift_df.columns:

        control_ref = (
            shift_df[
                shift_df["IsControlReference"].fillna(False)
            ]
            .groupby(
                [
                    "SubtypeClean",
        ...
        mask_control = (
            summary["SubtypeClean"].astype(str) == "control"
        ) & summary["ControlRelativeSTD"].notna()
...
def plot_saxs_total_collagen_4region_matched_percent_shift_with_stats(
    saxs_points,
    *,
    parameter="curvearea",
    normalise=True,
    ...
    """
    Plot matched % shift in SAXS collagen intensity:
    ...
    Unwounded:
        Included as a 0% reference with relative STD error bars.
    ...
    shift_df, summary = calculate_saxs_total_collagen_4region_matched_percent_shift(
        saxs_points=saxs_points,
        parameter=parameter,
        normalise=normalise,
        subtype_order=subtype_order,
    ...
    if shift_df.empty or summary.empty:
        print("No SAXS matched 4-region shift data to plot.")
        return None, None, shift_df, summary, pd.DataFrame()
    ...
    # Plot
    # ------------------------------------------------------------
    x_centres = np.arange(len(present_subtypes), dtype=float)

    # Add visual gap after Unwounded if present
    if "control" in present_subtypes:
        for i, st in enumerate(present_subtypes):
            if i > present_subtypes.index("control"):
                x_centres[i] += 1.0
    ...
    for j, shift_type in enumerate(shift_order):
        ...
        for subtype in present_subtypes:
            ...
            row = summary[
                (summary["SubtypeClean"].astype(str) == str(subtype))
                & (summary["ShiftType"].astype(str) == str(shift_type))
            ]
        ...
        if hatches and shift_type == "upper_shift":
            for bar in bars:
                bar.set_hatch("//")

        for subtype, xpos in zip(present_subtypes, xs):
            bar_positions[
                (
                    subtype,
                    shift_type,
    ...
    # ------------------------------------------------------------
    # Paired t-tests: lower vs upper within subtype
    # ------------------------------------------------------------
    if run_stats:
            ...
            rows.append(
                {
                    ...
                    "SelectedTest": "paired t-test",
                    "n_pairs": n_pairs,
                    "MeanLower": float(wide["lower_shift"].mean()) if n_pairs else np.nan,
                    "MeanUpper": float(wide["upper_shift"].mean()) if n_pairs else np.nan,
                    "MeanDifference_Upper_minus_Lower": (
                        float((wide["upper_shift"] - wide["lower_shift"]).mean())
                        if n_pairs
                        else np.nan
        ...
        if print_stats:
            print("\n[SAXS matched lower vs upper % shift paired t-tests]")
            if stats_df.empty:
                print("No paired t-tests were run.")
            else:
                print(stats_df.to_string(index=False))
    ...
    # ------------------------------------------------------------
    # Optional annotations lower vs upper within each subtype
    # ------------------------------------------------------------
    if annotate_stats and not stats_df.empty:
        ...
        for _, row in stats_df.iterrows():
            ...
            lower_summary = summary[
                (summary["SubtypeClean"].astype(str) == str(subtype))
                & (summary["ShiftType"].astype(str) == "lower_shift")
            ]

            upper_summary = summary[
                (summary["SubtypeClean"].astype(str) == str(subtype))
                & (summary["ShiftType"].astype(str) == "upper_shift")
            ]

            if lower_summary.empty or upper_summary.empty:
                continue

            y1 = (
                float(lower_summary["mean"].iloc[0])
                + float(lower_summary["std"].iloc[0])
            )
            y2 = (
                float(upper_summary["mean"].iloc[0])
                + float(upper_summary["std"].iloc[0])
            )

            y_line = max(y1, y2) + 0.05 * y_range
            h = 0.025 * y_range
...
def plot_raman_weighted_moment_percent_shift_from_control(
    raman_wm_long,
    *,
    metric="area_w",
    peak_regions=None,
    ...
    """
    Bar plot of Raman weighted moment % shift from control.

    Designed for metrics such as:
        area_w
        max_intensity
        mu2
        sigma
    ...
    Calculation is performed at sample level:

        % shift from control =
            ((sample value - control mean for that peak region)
             / control mean for that peak region) * 100

    x-axis = peak region
    grouped bars = subtype
    points = individual sample-level % shift values
    ...
    raman_wm_long : pd.DataFrame
        ...
        Expected columns:
            Sample, Subtype, PeakRegion, Metric, Value

    metric : str
        Raman weighted moment metric to plot, e.g. "area_w".

    peak_regions : list
        Can be:
            - list of names
            - RAMAN_PEAK_REGIONS as list of tuples:
                [("AmideIII_1410_1500", (1410, 1500)), ...]

    control_subtype : str
        Subtype used as baseline, e.g. "CT".
    """
    ...
    if missing:
        raise KeyError(
            f"Missing required columns for Raman % shift plot: {missing}"
        )

    d["Sample"] = d["Sample"].astype(str).str.strip()
    d["Subtype"] = d["Subtype"].astype(str).str.strip()
    d["PeakRegion"] = d["PeakRegion"].astype(str).str.strip()
    d["Metric"] = d["Metric"].astype(str).str.strip()
    d["Value"] = pd.to_numeric(d["Value"], errors="coerce")
    ...
    if d.empty:
        print(f"No Raman data found for metric={metric}.")
        return None, None, pd.DataFrame(), pd.DataFrame()

    # ------------------------------------------------------------
    # Resolve peak region names
    # ------------------------------------------------------------
    if peak_regions is not None:
        peak_names = []
        ...
        for p in peak_regions:
            if isinstance(p, tuple):
                peak_names.append(str(p[0]))
            else:
                peak_names.append(str(p))

        d = d[
            d["PeakRegion"].isin(peak_names)
        ].copy()

    else:
        peak_names = list(
            pd.unique(d["PeakRegion"])
        )

    if d.empty:
        print("No Raman data left after filtering peak regions.")
        return None, None, pd.DataFrame(), pd.DataFrame()

    peak_names = [
        p for p in peak_names
        if p in set(d["PeakRegion"])
    ]

    subtypes = [
        st for st in subtype_order
        if st in set(d["Subtype"])
    ]

    if control_subtype not in set(d["Subtype"]):
        print(f"Control subtype '{control_subtype}' not found in data.")
        print("Available subtypes:")
        print(sorted(pd.unique(d["Subtype"])))
        return None, None, pd.DataFrame(), pd.DataFrame()

    # ------------------------------------------------------------
    # Calculate control mean for each peak region
    # ------------------------------------------------------------
    control_means = (
        d[d["Subtype"] == control_subtype]
        .groupby("PeakRegion", observed=True)["Value"]
        .mean()
        .rename("ControlMean")
        .reset_index()
    )
    ...
    d["PercentShift"] = np.where(
        d["ControlMean"] != 0,
        ((d["Value"] - d["ControlMean"]) / d["ControlMean"]) * 100,
        np.nan,
    )
    ...
    if d.empty:
        print("No valid % shift values after control normalisation.")
        return None, None, pd.DataFrame(), pd.DataFrame()
    ...
    for peak in peak_names:
        for st in subtypes:
            ...
            if error_mode.lower() == "std":
                err = std
            elif error_mode.lower() == "sem":
                err = std / np.sqrt(vals.size)
            else:
                raise ValueError("error_mode must be 'sem' or 'std'.")
            ...
            rows.append(
                {
                    ...
                    "n": vals.size,
                    "Metric": metric,
                    "ControlSubtype": control_subtype,
                }
            )
    ...
    if ylabel is None:
        ylabel = f"{metric} shift from {control_subtype} (%)"

    if title is None:
        title = f"Raman {metric} % shift from {control_subtype}"
    ...
    if print_summary:
        print(f"\n[Raman {metric} % shift from {control_subtype}]")
        print(
            summary[
                [
                    "PeakRegion",
        ...
        print("\n[Sample-level Raman % shift values]")
        print(
            d[
                [
                    "Sample",
...
def plot_raman_pca_scores_2x2_by_normpos(
    scores_df,
    *,
    subtype_order=("ts", "vh", "ac", "cl"),
    pcx="PC1",
    pcy="PC2",
    norm_col="NormPos",
    cmap="jet",
    point_size=40,
    alpha=0.8,
    title="Raman PCA scores by normalised position",
    figsize=(10, 8),
    sharex=False,
    sharey=False,
):
    """
    Make a 2x2 plot of Raman PCA scores.

    Each panel = one subtype.
    Points = individual Raman point spectra.
    Colour = normalised position through the Raman line scan.

    This function uses an existing PCA scores table, e.g. the
    raman_pca_scores output from plot_raman_pca_within_subtype().
    """

    if scores_df is None or scores_df.empty:
        print("No Raman PCA scores available.")
        return None, None
    ...
    if missing:
        raise KeyError(
            f"Missing required PCA score columns: {missing}"
        )

    d = scores_df.copy()

    d["Subtype"] = d["Subtype"].astype(str).str.strip()
    d[pcx] = pd.to_numeric(d[pcx], errors="coerce")
    d[pcy] = pd.to_numeric(d[pcy], errors="coerce")
    d[norm_col] = pd.to_numeric(d[norm_col], errors="coerce")
    ...
    if d.empty:
        print("No usable PCA score rows after cleaning.")
        return None, None

    present = set(d["Subtype"].astype(str))

    subtypes = [
        st for st in subtype_order
        if str(st) in present
    ]

    if not subtypes:
        print("No matching subtypes found in PCA scores.")
        print("Available subtypes:")
    ...
    for ax, subtype in zip(axes, subtypes):
        ...
        ds = d[
            d["Subtype"].astype(str) == str(subtype)
        ].copy()

        label = (
            subtype_label(subtype)
            if "subtype_label" in globals()
            else str(subtype)
        )
    ...
    if last_scatter is not None:
        ...
        cbar.set_label(
            "Normalised position through line scan (%)"
        )
...
def plot_raman_pca_with_normpos_as_feature_2x2(
    spectra,
    *,
    subtype_order=("ts", "vh", "ac", "cl"),
    wave_col="WaveRound",
    ...
    point_size=40,
    alpha=0.8,
    title="Raman PCA including normalised position as a feature",
    figsize=(10, 8),
):
    """
    Run PCA separately within each subtype using:

        Raman spectral intensities across wavenumbers
        +
        normalised position through the line scan as an additional feature

    Each row = one Raman point spectrum.
    Columns = wavenumber intensities + NormPos feature.

    The 2x2 output plots PC1 vs PC2.
    Points are coloured by NormPos using JET.

    normpos_weight:
        Multiplier applied to the NormPos feature before PCA.
        Use 1.0 by default.
        Increase if you want spatial position to contribute more strongly.
        Decrease if you want it to contribute less strongly.
    """
    ...
    if meta.empty or Y.size == 0:
        print("No Raman spectra available after matrix conversion.")
        return None, None, pd.DataFrame(), {}

    if norm_col not in meta.columns:
        print(f"'{norm_col}' not found in PCA metadata.")
        print("Available metadata columns:")
        print(meta.columns.tolist())
        return None, None, pd.DataFrame(), {}

    meta[norm_col] = pd.to_numeric(
        meta[norm_col],
        errors="coerce",
    )

    keep = np.isfinite(meta[norm_col].to_numpy(float))

    meta = meta.loc[keep].reset_index(drop=True)
    Y = Y[keep, :]

    if meta.empty or Y.size == 0:
        print("No spectra with valid normalised position.")
        return None, None, pd.DataFrame(), {}

    present = list(
        pd.unique(
            meta["Subtype"].astype(str)
        )
    )
    ...
    if not subtypes:
        print("No matching subtypes for PCA.")
        print("Available subtypes:")
        print(present)
        return None, None, pd.DataFrame(), {}
    ...
    for ax, subtype in zip(axes, subtypes):
        ...
        meta_sub = meta.loc[idx].copy().reset_index(drop=True)
        Y_sub = Y[idx, :]
        ...
        norm_feature = norm_feature * float(normpos_weight)
        ...
        if pca is None:
            ax.set_title(f"{subtype}: PCA failed")
            ax.axis("off")
            continue
        ...
        for i in range(min(scores.shape[1], 6)):
            meta_sub[f"PC{i + 1}"] = scores[:, i]

        meta_sub["PCA_Mode"] = "within_subtype_with_normpos_feature"
        meta_sub["PCA_Subtype"] = subtype
        ...
        feature_names = list(wave_grid.astype(str)) + [norm_col]
        ...
        pca_info[subtype] = {
            ...
            "explained_variance_ratio": evr,
            "standardised": standardise,
            "included_normpos_feature": True,
            "normpos_weight": normpos_weight,
        }
    ...
    return fig, axes, scores_df, pca_info
...
raman_df = combined_data["Raman"]
raw_raman_df = combined_data["RawRaman"]
cell_df = combined_data["Cell"]
saxs_df = combined_data["SAXS"]
ni_df = combined_data["Nanoindentation"]
raw_ni_df = combined_data["RawNanoindentation"]
...
fig, ax, ni_modulus_smooth = plot_ni_raw_linescan_by_subtype_smooth(
    ...
    ylabel="Indentation modulus (kPa)",
    title=" ",
    subtypes_to_plot=["control", "d7", "d14", "d21"],
    window=18,
    grid_size=200,
...
fig, ax, cell_spatial_summary, cell_spatial_ttests = plot_cell_spatial_4region_bar(
    ...
    subtypes_to_plot=["control", "d7", "d10", "d14", "d21"],
    ylim=None,
    hatches=True,
    add_ttests=True,
)
...
fig, ax = plot_cell_dermis_vs_wound_bar(
    cell_raw=tables["cell_raw"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=False,
)
...
fig, ax, cell_shift_df, cell_shift_summary, cell_shift_stats = plot_cell_wound_vs_dermis_shift(
    cell_raw=tables["cell_raw"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=False,
    run_stats=True,
...
fig, ax, cell_dw_shift_df, cell_dw_shift_summary, cell_dw_shift_stats = plot_cell_dermis_to_wound_percent_shift_anova(
    cell_raw=tables["cell_raw"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=False,
    errorbar="std",
...
fig, ax, cell_dw_shift_df, cell_dw_shift_summary, cell_dw_overall_stats, cell_dw_pairwise_stats = plot_cell_dermis_to_wound_percent_shift_anova(
    cell_raw=tables["cell_raw"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=False,
    errorbar="sem",
...
fig, ax, saxs_dw_shift_df, saxs_dw_shift_summary, saxs_dw_overall_stats, saxs_dw_pairwise_stats = plot_saxs_dermis_to_wound_percent_shift_anova(
    saxs_points=tables["saxs_points"],
    parameter="curvearea",
    normalise=True,
    trim_std_devs=6,
...
fig, ax = plot_saxs_total_collagen_4region(
    saxs_points=tables["saxs_points"],
    parameter="curvearea",
    normalise=True,
    trim_std_devs=6,
...
fig, ax = plot_saxs_dperiod_dermis_vs_wound(
    saxs_points=tables["saxs_points"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=False,
)
...
fig, ax = plot_saxs_dperiod_4region(
    saxs_points=tables["saxs_points"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=True,
)
...
fig, ax, saxs_dperiod_shift_df, saxs_dperiod_shift_summary, saxs_dperiod_overall_stats, saxs_dperiod_pairwise_stats = plot_saxs_dperiod_shift_dermis_to_wound(
    saxs_points=tables["saxs_points"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=False,
    run_stats=True,
...
fig, ax = plot_saxs_dperiod_fwhm_dermis_vs_wound(
    saxs_points=tables["saxs_points"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=False,
)

fig, ax, saxs_fwhm_shift_df, saxs_fwhm_shift_summary, saxs_fwhm_overall_stats, saxs_fwhm_pairwise_stats = plot_saxs_dperiod_fwhm_dermis_to_wound_percent_shift_anova(
    saxs_points=tables["saxs_points"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=False,
    errorbar="std",
...
fig, ax = plot_saxs_dperiod_fwhm_4region(
    saxs_points=tables["saxs_points"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=True,
)
...
fig, ax = plot_saxs_peak_position_bar(
    saxs_points=tables["saxs_points"],
    min_rsq=0.3,
    ylim=(0, 180),
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
...
fig, ax, saxs_peak_shift_df, saxs_peak_shift_summary, saxs_peak_overall_stats, saxs_peak_pairwise_stats = plot_saxs_peak_position_shift_from_dermis(
    saxs_points=tables["saxs_points"],
    min_rsq=0.3,
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=False,
...
fig, ax = plot_saxs_peak_spread_dermis_vs_wound(
    saxs_points=tables["saxs_points"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=False,
)
...
fig, ax = plot_saxs_peak_spread_4region(
    saxs_points=tables["saxs_points"],
    subtypes_to_plot=["control", "d4", "d7", "d10", "d14", "d21"],
    hatches=True,
)
...
fig, ax, saxs_4region_shift_df, saxs_4region_shift_summary, saxs_4region_shift_stats = plot_saxs_total_collagen_4region_matched_percent_shift_with_stats(
    saxs_points=tables["saxs_points"],
    parameter="curvearea",
    normalise=True,
    trim_std_devs=6,
...
fig, ax, raman_selected_wm = plot_raman_selected_weighted_moments(
    raman_wm_long,
    peak_regions=["AMIDEIII_RIGHT_1330_1380"],
...
plot_RAMAN_PEAK_REGIONS = [
    ("AMIDEIII_RIGHT_1330_1380", (1310, 1380)),
    ("AmideI_LEFT_1530_1590", (1530, 1589)),
    ("AmideI_MIDDLE_1590_1635", (1590, 1634)),
    ("AmideI_RIGHT_1635_1700", (1635, 1700)),
]
fig, ax, raman_area_shift_values, raman_area_shift_summary = plot_raman_weighted_moment_percent_shift_from_control(
    raman_wm_long,
    metric="area_w",
    peak_regions=plot_RAMAN_PEAK_REGIONS,
    subtype_order=["CT", "D7", "D14"],
    colours=RAMAN_COLOURS,
    error_mode="sem",
    control_subtype="CT",
    ylabel="Peak area shift from control (%)",
    title="Raman peak area % shift from control",
)
